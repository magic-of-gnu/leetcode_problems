...
class Solution:
    ...
    def largestDivisibleSubset(self, nums):
        ...
        _sorted = sorted(nums, reverse=True)
        print(_sorted)
        ...
        for idx, val in enumerate(_sorted[1:], start=1):
            # print()
            ...
            if not results:
                ...
                for ii in range(len(results)):
                    # first value of every possible set
                    res = results[ii]
                    # print(f'here: {res}')
                    # print(f'val: {val}')
                    # print(f'arr.0: {res["arr"][0]}')
                    # if res['arr'][0] % val == 0:
                    #     rres = {
                    #         'arr': deepcopy(res['arr']),
                    #         'len': res['len'] + 1
                    #     }
                    #     rres['arr'].appendleft(val)
                    #     is_appended = True
                    #     print(rres)
                    #     new_results.append(rres)

                    #     if rres['len'] > max_length:
                    #         print(f'new largest, before: {idx_largest}')
                    #         idx_largest = len(results) - 1 + len(new_results)
                    #         print(f'new largest, after: {idx_largest}')
                    #         max_length = rres['len']

                    if res['arr'][0] % val == 0:
                        res['arr'].appendleft(val)
                        res['len'] += 1
                        is_appended = True

                        used.union(set(res['arr']))
                ...
                for newval in _sorted[:idx]:
                    if newval in used:
                        continue
                    newset = deque()
                    if newval % val == 0:
                        newset = deque([val, newval])
                        is_appended = True
                    ...
                    if results[-1]['len'] > max_length:
                        idx_largest = len(results) - 1

                if is_appended is False:
                    newset = deque()
                    newset.appendleft(val)
                    results.append({
                        'arr': newset,
                        'len': len(newset)
                     })
        ...
        print(f'idx_largest: {idx_largest}')
        # print(results)
        print(len(results))

        return results[idx_largest]['arr']
